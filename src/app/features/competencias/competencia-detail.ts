...
import {
  Competencia, EstadoCompetencia, FaseDisciplinaConfig,
  ESTADO_LABELS, TIPO_LABELS, MODALIDAD_LABELS, ESTRUCTURA_LABELS, DIAS_SEMANA_LABELS,
} from '../../core/models/competencia.model';
import { Encuentro, EstadoEncuentro, FechaCompetencia, FASE_LABELS } from '../../core/models/encuentro.model';
          ...
          <div class="section-card p-4">
            ...
            @if (c.disciplinaIds.length > 0) {
                ...
                @for (dId of c.disciplinaIds; track dId) {
                  <div class="rounded-xl border border-green-100 bg-green-50/40 px-3 py-2">
                    ...
                    <div class="flex flex-wrap gap-1.5 mt-2">
                      @for (fase of getFasesDisciplina(c.id, dId); track fase.id) {
                        <span class="inline-flex items-center rounded-lg border border-amber-200 bg-white px-2 py-0.5 text-[11px] font-semibold text-amber-700">
                          {{ fase.nombre }} · {{ faseLabelsMap[fase.tipo_fase] }}
                        </span>
                      }
                    </div>
                  </div>
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <a [routerLink]="['editar']" class="btn-primary !text-xs !px-3 !py-1.5">Agregar disciplinas</a>
              </div>
            }
          </div>
        </div>
...
export class CompetenciaDetailComponent implements OnInit {
  ...
  protected getFasesDisciplina(competenciaId: string, disciplinaId: string): FaseDisciplinaConfig[] {
    const config = this.competenciaService
      .getDisciplinasConfig(competenciaId)
      .find((item) => item.disciplinaId === disciplinaId);
    if (!config) return [];
    return config.pruebas.flatMap((prueba) => prueba.fases);
  }
