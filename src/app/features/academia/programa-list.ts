  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a [routerLink]="['/', { outlets: { primary: ['academia', 'programas'], panel: ['academia', 'programas', 'nuevo'] } }]" class="inline-flex items-center gap-2 bg-brand text-white px-4 py-2 rounded-lg hover:bg-brand-600 transition-colors text-sm font-medium">
        ...
        @for (prog of programas(); track prog.id) {
          <div class="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div class="flex items-start justify-between">
              <div class="min-w-0">
                <a [routerLink]="[prog.id]" class="text-lg font-semibold text-green-600 hover:text-green-800">
                  {{ prog.nombre }}
                </a>
            ...
            <div class="mt-4 pt-4 border-t flex items-center justify-end">
              <a [routerLink]="[prog.id]" class="text-green-600 hover:text-green-800 text-sm font-medium">
                Ver detalle →
              </a>
            </div>
          </div>
