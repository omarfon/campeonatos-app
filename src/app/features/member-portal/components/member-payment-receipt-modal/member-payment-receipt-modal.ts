...
import { MEMBER_PORTAL_NAME } from '../../member-portal.constants';

@Component({
  selector: 'app-member-payment-receipt-modal',
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    @if (open() && payment(); as p) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
        <button type="button" class="absolute inset-0 bg-slate-900/50 backdrop-blur-[2px]"
          aria-label="Cerrar comprobante" (click)="close.emit()"></button>
        ...
        <div class="relative w-full max-w-md max-h-[90vh] overflow-hidden rounded-2xl shadow-2xl flex flex-col animate-[scaleIn_0.2s_ease-out] bg-white"
          ...
          <div class="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-3 shrink-0">
            ...
            <button type="button" class="p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors shrink-0"
              aria-label="Cerrar modal" (click)="close.emit()">
          ...
          <div class="flex-1 overflow-y-auto p-5 space-y-5">
            <div class="rounded-xl border border-dashed border-slate-200 p-4 space-y-4 bg-slate-50/50">
              ...
              <dl class="space-y-3 text-sm">
                ...
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-500 shrink-0">Titular</dt>
                  <dd class="font-semibold text-slate-900 text-right">{{ memberName() }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-500 shrink-0">Participante</dt>
                  <dd class="font-semibold text-slate-900 text-right">{{ p.participantName }}</dd>
                </div>
                <div class="flex justify-between gap-4">
                ...
                @if (p.activityName) {
                  <div class="flex justify-between gap-4">
                    <dt class="text-slate-500 shrink-0">Actividad</dt>
                    <dd class="font-semibold text-slate-900 text-right">{{ p.activityName }}</dd>
                  </div>
                }
                @if (p.method) {
                  <div class="flex justify-between gap-4">
                    <dt class="text-slate-500 shrink-0">Método</dt>
                    <dd class="font-semibold text-slate-900 text-right">{{ p.method }}</dd>
                  </div>
                }
                <div class="flex justify-between gap-4">
                  <dt class="text-slate-500 shrink-0">Código</dt>
            ...
            <p class="text-xs text-center text-slate-500">Documento generado electrónicamente.</p>
          </div>
...
export class MemberPaymentReceiptModalComponent {
  private readonly sessionService = inject(MemberSessionService);

  readonly open = input(false);
  readonly payment = input<MemberPayment | null>(null);
  readonly close = output<void>();

  protected readonly portalName = MEMBER_PORTAL_NAME;
  protected readonly titleId = `member-receipt-${Math.random().toString(36).slice(2, 9)}`;

  protected readonly memberName = computed(
    () => this.sessionService.session()?.fullName ?? 'Socio',
  );
